    ...
    while (true)
    {
        read(touchScreenInfo.fd, &ie, sizeof(ie));
        {
            ...
            if (ie.type == EV_SYN)
            {
                ...
                {
                    Vector2 point = rotatePointx({(float) eventX, (float) eventY}, {screenInfo.width, screenInfo.height});
                    ImVec2 newEvent(point.x * screenToTouchRatio, point.y * screenToTouchRatio);
                    io.MousePos = newEvent;
                    upLoad();
                    continue;
                }
                continue;
